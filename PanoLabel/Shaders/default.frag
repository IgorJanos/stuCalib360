...
vec2 distort(vec2 p, float rMax)
{
    float r = length(p);
...
void main()
{
    ...
    // Center coordinates rescaled with aspect ratio
    vec2 i = (t - vec2(0.5, 0.5)) * canvas;

    // compute distortion & reprojection
    vec2 distortedPos = distort(i, rMax);
